...
if __name__ == '__main__':
    ...
    for line in fileObj:
        ...
        if (len(key_value) == 2):
            params[key_value[0].strip()] = key_value[1]
    testUsersNum = int(params['testUsersNum'])  # 实验用户的个数
    ...
    print("--------------------------最终结果-------------------------")
    # print("精确率：", precision / testIters)
    # print("平均CTR率：", HR / testIters)
    # print("平均F1", F1 / testIters)
    # print("推荐累计奖励", reward / testIters)
    print("精确率：", PrecisionRes)
    print("平均Recall率：", recallRes)
    print("平均F1", F1Res)
    print("推荐累计奖励", RewardRes)
    print("实验用户个数", testUsersNum)
    print("每个用户实验的次数", rounds)
    print("实验轮数", testIters)
