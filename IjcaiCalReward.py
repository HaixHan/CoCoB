...
class Payoff:
    def get_reward(self, t, item, data):
        ...
        n = len(user_data)
        m = user_data[(user_data['item_id'] == t.item_id) & (user_data['cat_id'] == t.cat_id) & (
                    user_data['time_stamp'] == t.time_stamp)].index.values[0]
        verification = user_data[m + 1:n]
        Hit = []
        flag = 0
        if len(verification[verification['item_id'] == item]) > 0:
            flag = 1
            interact_data = verification[verification['item_id'] == item][
                'action_type'].value_counts().to_frame().reset_index()
